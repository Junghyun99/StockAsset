# tests/test_spy_engine.py
"""SpyEngine 단위 테스트.

SPY Buy&Hold 벤치마크를 시뮬레이션하는 엔진:
- A그룹: [SPY], B그룹: [SHV]
...
from src.core.engine import SpyEngine, FullExposureEngine, _ENGINE_REGISTRY as ENGINE_REGISTRY
...
def _build_spy_engine(repo_last_reb=None):
    broker = MagicMock()
    repo = MagicMock()
    logger = MagicMock()
    ...
    broker.get_portfolio.return_value = Portfolio(
        total_cash=10000.0,
        holdings={"SPY": 20},
        current_prices={"SPY": 520.0, "SHV": 110.0},
    )
...
def test_spy_engine_asset_groups_A():
    """SpyEngine의 A그룹은 [SPY]."""
    assert SpyEngine.ASSET_GROUPS['A'] == ['SPY']


def test_spy_engine_asset_groups_B():
    """SpyEngine의 B그룹은 [SHV]."""
    assert SpyEngine.ASSET_GROUPS['B'] == ['SHV']


def test_spy_engine_no_C_group():
    """SpyEngine에는 C그룹이 없다."""
    assert 'C' not in SpyEngine.ASSET_GROUPS


def test_spy_engine_rebalance_ratio_a():
    """REBALANCE_RATIO_A는 0.999로 사실상 100% SPY 투자."""
    assert SpyEngine.REBALANCE_RATIO_A == 0.999


def test_spy_engine_is_full_exposure_subclass():
    """SpyEngine은 FullExposureEngine의 서브클래스다."""
    assert issubclass(SpyEngine, FullExposureEngine)
...
def test_spy_engine_registered_in_registry():
    """SpyEngine이 ENGINE_REGISTRY에 등록되어 있다."""
    names = [name for name, _ in ENGINE_REGISTRY]
    assert "SpyEngine" in names


def test_spy_engine_registry_class_is_spy_engine():
    """레지스트리에서 'SpyEngine' 이름으로 SpyEngine 클래스가 반환된다."""
    for name, cls in ENGINE_REGISTRY:
        if name == "SpyEngine":
            assert cls is SpyEngine
            break
...
def test_spy_engine_rebalancer_groups():
    """rebalancer가 SpyEngine.ASSET_GROUPS로 자동 생성된다."""
    ...
    with patch('src.core.engine.IndicatorCalculator'), \
        ...
        engine = SpyEngine(broker=broker, repo=repo, logger=logger)
    assert engine.rebalancer.groups == SpyEngine.ASSET_GROUPS


def test_spy_engine_rebalancer_ratio_a():
...
def test_spy_engine_rebalancer_ratio_b():
...
def test_spy_engine_all_tickers():
    """all_tickers는 SPY + SHV 조합이다."""
    engine, _ = _build_spy_engine()
    assert set(engine.all_tickers) == {"SPY", "SHV"}
...
def test_spy_engine_bull_exposure_1():
    """BULL 국면에서 exposure=1.0."""
    engine, mocks = _build_spy_engine()
...
def test_spy_engine_crash_exposure_1():
...
def test_spy_engine_bear_strong_exposure_1():
...
def test_spy_engine_nan_exposure_zero():
...
def test_spy_engine_does_not_call_targeter():
...
def test_spy_engine_end_to_end_rebalancing():
    """전체 사이클: exposure=1.0이 rebalancer에 전달된다."""
    engine, mocks = _build_spy_engine(repo_last_reb=None)
...
def test_spy_engine_end_to_end_nan_no_trade():
