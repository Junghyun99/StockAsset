...
| 분할 금액 기준 | **트리거 시점에 `총자산×비중`을 고정**해 N등분 (큐). 실제 체결은 가용현금으로 캡 |
...
1. **매수 트리거 평가** (종가 기준, 밴드폭 `band`=0.02):
   ...
   - 매수 사이징 기준 = **트리거 시점 총자산**(현금+보유평가액). '남은 현금의 %'는
     앞선 레벨이 현금을 소진해 깊은 레벨이 작아지는 비대칭이 있어, 풀사이클
     백테스트에서 우위였던 총자산 기준으로 채택. 실제 체결은 가용현금으로 캡.
   - `ma20` 밴드 진입 & `armed.ma20` → 매수 트랜치(총자산×0.10, 1일) 적재, `armed.ma20=False`
   - `ma60` 밴드 진입 & `armed.ma60` → 매수 트랜치(총자산×0.50, 5일) 적재, `armed.ma60=False`
   - `ma120` 밴드 진입 & `armed.ma120` → 매수 트랜치(총자산×0.50, 5일) 적재, `armed.ma120=False`
   - `price < ma120` & `rsi < 30` & `armed.dip` → 매수 트랜치(총자산×1.00, 40일) 적재, `armed.dip=False`
